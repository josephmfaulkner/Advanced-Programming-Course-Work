...
string trim_string(string& input_string){
    size_t first_char = input_string.find_first_not_of (" \t");
    size_t last_char = input_string.find_last_not_of (" \t");
    if (first_char == string::npos)
        return "";
    else
        return input_string.substr (first_char,
                                    last_char - first_char + 1);

}

bool line_is_comment(string& test_string){
    if( test_string.size() == 0 or test_string[0]=='#')
        return true;
    else
        return false;
}

bool line_has_equals(string& test_string){
    string eq = string("=");
    size_t first_eq = test_string.find_first_of(eq);
...
string line_get_key(string& test_string){
    string eq = string("=");
    size_t first_eq = test_string.find_first_of(eq);
    return test_string.substr(0, first_eq);



}

string line_get_value(string& test_string){
    string eq = string("=");
    size_t first_eq = test_string.find_first_of(eq);
    return test_string.substr(first_eq+1,string::npos - first_eq);
}
